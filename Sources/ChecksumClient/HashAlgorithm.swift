...
/// instead of providing the whole buffer up front. The abstraction is necessary
/// because on Apple platforms like macOS we can take advantage of `CryptoKit`,
/// but on Linux we must fall back to our own implementation.
///
/// This is partially inspied by [this implementation](https://github.com/swiftlang/swift-tools-support-core/blob/main/Sources/TSCBasic/HashAlgorithms.swift)
/// in Swift Package Manager. The API is very similar to the `SHA256` API in `CryptoKit`.
...
struct CryptoKitSHA256: HashAlgorithm, @unchecked Sendable {
    private var sha256 = CryptoKit.SHA256()
...
struct InternalSHA256: HashAlgorithm {
    ...
    private static let blockByteSize = blockBitSize / 8
    ...
    private var leftoverBuffer = [UInt8]()
    private var hash = Self.initialHashValue
